 ...
 */
  ...
  if (--sched_ticks == 0)
  {
    ...
    if ((rdy_head[USER_Q]->p_group) % 2 == GROUP_A)
      sched_ticks = proc_speed * SCHED_RATE / 50; /* reset quantum */
    else
      sched_ticks = (100 - proc_speed) * SCHED_RATE / 50;
 ...
 */
  ...
  if (!istaskp(rp))
    function = 0; /* user processes get signaled */
 ...
 *				common_setalarm				     *
 *===========================================================================*/
PRIVATE void common_setalarm(proc_nr, delta_ticks, function) int proc_nr; /* which process wants the alarm */
long delta_ticks;                                                         /* in how many clock ticks does he want it? */
 ...
 *				clock_handler				     *
 *===========================================================================*/
PRIVATE int clock_handler(irq) int irq;
{
 ...
 */
  ...
  if (rp != bill_ptr && rp != proc_addr(IDLE))
    bill_ptr->sys_time += ticks;

  pending_ticks += ticks;
  now = realtime + pending_ticks;
  if (tty_timeout <= now)
    tty_wakeup(now); /* possibly wake up TTY */
  ...
  if (--sched_ticks == 0)
  {
    /*printf("Clock handler, grupa = %d\n", rdy_head[USER_Q]->p_group)*/;
    /* If bill_ptr == prev_ptr, no ready users so don't need sched(). */
    if ((rdy_head[USER_Q]->p_group) % 2 == GROUP_A)
      sched_ticks = proc_speed * SCHED_RATE / 50;
    else
      sched_ticks = (100 - proc_speed) * SCHED_RATE / 50;
    prev_ptr = bill_ptr;      /* new previous process */
  }
 ...
 *				milli_delay				    *
 *==========================================================================*/
PUBLIC void milli_delay(millisec) unsigned millisec;
{
 ...
 *				milli_start				    *
 *==========================================================================*/
PUBLIC void milli_start(msp) struct milli_state *msp;
{
 ...
 *				milli_elapsed				    *
 *==========================================================================*/
PUBLIC unsigned milli_elapsed(msp) struct milli_state *msp;
{
