...
FORWARD _PROTOTYPE( int do_setscheduler, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      case SYS_SETSCHEDULER:  r = do_setscheduler(&m); break;
	    default:		r = E_BAD_FCN;
	}
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->p_group = (m_ptr->PID) %2; /* przydzial do grupy A lub B */
  /* if (rpc->p_priority == PPRI_USER)
    printf("Grupa = %d, PID = %d \n ", rpc->p_group, rpc->p_pid); */
  rpc->p_reg.retreg = 0;	/* child sees pid = 0 to know it is child */
  /* rpc->p_group = rpc->p_pid % 2; */ 
...
PUBLIC int do_getgroup(m_ptr)
message *m_ptr;
{
  printf("getgroup na poziomie taskcalla\n");
  return rdy_head[USER_Q]->p_group;
}

PRIVATE int do_setscheduler(m_ptr)
message *m_ptr;
{
  int i = 0;
  int pid = m_ptr->m1_i1;
  for(; i < NR_TASKS + NR_PROCS; ++i) {
    if (pproc_addr[i]->p_pid == pid) {
      if (pproc_addr[i]->p_group == GROUP_A) {
        proc_speed = m_ptr->m2_l2;
        printf("Grupa A, proc_speed ustawiony na %d\n", proc_speed);
        return 0;
      }
      printf("Grupa B, nie udalo sie ustawic proc_speed na %d, aktualna wartosc proc_speed %d\n", m_ptr->m2_l2, proc_speed);
      return -1;
    }
  }
}
